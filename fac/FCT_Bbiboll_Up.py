...
class  FCT_Bbiboll_Up(FactorTemplate):
    def __init__(self):
        super().__init__()
        self.factor_name = 'FCT_Bbiboll_Up'
    ...
    def formula(self,**param):
        ...
        df['UPR'] = df['BBI'] + df['BBI'].rolling(window=Length).std()
        df['sc'] = (df['close'] - df['UPR']) / df['open']
