...
该模块负责计算和分析量化交易策略的各项绩效指标，包括收益率、风险指标、
交易统计等，并生成可视化图表和HTML报告。
...
# # 设置中文字体
# TODO: 这里的font_manager找不到引用，需要修正。
...
class Performance():
    ...
    负责计算量化交易策略的各项绩效指标，包括收益、风险、交易统计等，
    并生成可视化图表和HTML报告。
    ...
    def __init__(self, param):
        ...
        param : dict
            配置参数字典，包含：
            - st: 开始时间（YYYYMMDD格式字符串）
            - et: 结束时间（YYYYMMDD格式字符串）
            - initial_value: 初始资金
        ...
        self.data_common = Data_Common.create_instance()
        # 持仓记录
        self.equity_order = pd.DataFrame(
            columns=['date', 'order_date', 'trading_date', 'instrument_id', 'instrument', 'buyOrSell', 'openOrClose',
                     'price', 'openPrice',
                     'count', 'count_back', 'ID', 'tradeId', 'mul', 'flag', 'model_flag', 'status', 'fee', 'pre_price',
                     'current_price'])
        # 每日持仓统计
        self.daily_df = pd.DataFrame(
            columns=["date", 'profit', 'model_flag', 'fee', 'long_fee', 'short_fee', 'profit_all',
    ...
    def get_data(self, instrument=""):
        ...
        if instrument == "":
            # TODO: object未定义__getietm__，不能对这个实例使用[]运算符
            df = self.order_df[
                (self.order_df['model_flag'] == self.model_flag) & (self.order_df['trading_date'] >= st) & (
                            self.order_df['trading_date'] <= et)]
            return df
        else:
            df = self.order_df[
                (self.order_df['model_flag'] == self.model_flag) & (self.order_df['instrument'] == instrument)]
            return df
    ...
    def profit(self):
        ...
        for item in self.ins_list.copy():
            ...
            self.obj[ins]["long_profit"] = (
                        (long_df['price'] - long_df['openPrice']) * long_df['mul'] * long_df['count']).sum()
            ...
            self.obj[ins]["short_profit"] = (
                        (short_df['price'] - short_df['openPrice']) * short_df['mul'] * short_df['count'] * SELL).sum()
    ...
    @staticmethod
    def long_or_short_factors(df_temp):
        """
        计算多头或空头的盈利和亏损
        ...
        loss = 0
        df = df_temp.copy()
        df['profit_res'] = (df['price'] - df['openPrice']) * df['openOrClose'] * df['buyOrSell'] * df['mul'] * df[
            'count']
    ...
    def profit_factor(self):
        ...
        # 交易次数
        # TODO: 这里的几个sum似乎是bool类型，是否能直接用sum()进行加和
    ...
    def get_daily_records(self, date, pre_day):
        ...
        res_open = self.order_df[
            (self.order_df['model_flag'] == self.model_flag) & (self.order_df['trading_date'] == date) & (
                        self.order_df['openOrClose'] == OPEN)].sort_values(by="datetime")
        open_list = res_open.to_dict(orient='records')
        # 查询今天的平仓记录，判断开仓中是否有平仓
        res_close = self.order_df[
            (self.order_df['model_flag'] == self.model_flag) & (self.order_df['trading_date'] == date) & (
                        self.order_df['openOrClose'] == CLOSE)].sort_values(by="datetime")
        ...
        if len(self.daily_df) == 0:
            ...
            pre_equity_df = self.equity_order.loc[
                (self.equity_order['date'] == pre_day) & (self.equity_order['model_flag'] == self.model_flag)]
    ...
    def calc_profit_float_and_all(self):
        ...
        for idx, doc in equity_list.iterrows():
            ...
            for i, item in orders.iterrows():
                profit_float += ((item['price'] - item['openPrice']) * item['count_back']) * item['mul'] * item[
                    'buyOrSell']
            ...
            self.daily_df.loc[idx, ['profit_float', 'profit_all', 'rate', 'sum_profit', 'sum_rate']] = [profit_float,
                                                                                                        profit_all,
                                                                                                        rate,
                                                                                                        sum_profit,
                                                                                                        sum_rate]
        ...
        for idx, doc in equity_list.iterrows():
            ...
            for i, item in orders.iterrows():
                if item['buyOrSell'] == BUY:
                    long_profit += ((item['price'] - item['openPrice']) * item['count_back']) * item['mul'] * item[
                        'buyOrSell']
                else:
                    short_profit += ((item['price'] - item['openPrice']) * item['count_back']) * item['mul'] * item[
                        'buyOrSell']
            # 统计的盈利减去每日的手续费，是每日的实际盈利
    ...
    def draw_plt(self):
        ...
        self._draw_plt(x, y5, y3, y4, '收益曲线', '日期', '收益率',
                       f'{base_file_store_path}/images/{self.image_path}_3.png')
        x_date_file = ["date"]
    ...
    @staticmethod
    def write_list_to_json(my_list, file_name):
        """
        将列表写入JSON文件
    ...
    def draw_single(self, data, ins, initial_value):
        ...
        self._draw_plt(x, y5, y3, y4, f'{ins}--收益曲线', '日期', '收益率',
                       f'{base_file_store_path}/images/{self.image_path}_{ins}.png')
        pass
    ...
    def json_2_file(self):
        ...
        with open(f'{file_store_path}/{self.html_name}.html', 'w', encoding='utf-8') as file:
            ...
            file.write(
                'date_set.push(data1[0]);date_set.push(data2[0]);date_set.push(data3[0]);date_set.push(data4[0]); \n')
            ...
            file.write('<div>')
            file.write(
                f'本金{self.initial_value} | 年化收益率:{round(self.obj["total_rate"] * 100, 2)}% | 总收益:{round(self.obj["total_profit"], 2)} | 费率比:{round(self.obj["rateRatio"], 2)}| 最大回撤:{round(self.obj["max_drawdown"], 2)}%|风险收益比: {fl}  |夏普：{self.obj["sharpe_ratio"]} </br> ')
            file.write(
                f'总交易次数: {self.obj["all_trade_count"]} | 多头次数: {self.obj["long_trade_count"]} | 空头次数: {self.obj["short_trade_count"]} </br> ')
            file.write(
                f'多头盈利: {self.obj["factor"]["long_profit"]} | 多头损失: {self.obj["factor"]["long_loss"]} | 空头盈利: {self.obj["factor"]["short_profit"]} | 空头损失: {self.obj["factor"]["short_loss"]} (本行统计不含手续费)</br> ')
            file.write(
                f'多头盈利: {self.obj["long_win_profit"]} | 多头损失: {self.obj["long_loss_profit"]} | 空头盈利: {self.obj["short_win_profit"]} | 空头损失: {self.obj["short_loss_profit"]} (本行统计含手续费)</br> ')
            file.write(
                f'多头净利润: {round(long_profit, 2)} | 空头净利润: {round(short_profit, 2)} | 总净利润: {round(long_profit + short_profit, 2)} </br>')
            file.write(f'多头盈利因子: {long_factore} | 空头盈利因子: {short_factore} </br>')

            file.write(f'手续费: {round(self.obj["fee"], 2)} </br>')

            file.write(
                f'盈利总次数: {self.obj["win_count"]} | 亏损总次数: {self.obj["loss_count"]} | 总胜率: {round(self.obj["win_rate"] * 100, 2)}% </br>')
            file.write(
                f'多头盈利次数: {self.obj["long_win_count"]} | 多头亏损次数: {self.obj["long_loss_count"]} | 多头胜率: {round(self.obj["long_win_rate"] * 100, 2)}% </br>')
            file.write(
                f'空头盈利次数: {self.obj["short_win_count"]} | 空头亏损次数: {self.obj["short_loss_count"]} | 空头胜率: {round(self.obj["short_win_rate"] * 100, 2)}% </br>')

            file.write(
                f'总平均持仓天数: {round(self.obj["total_avg_holding"], 2)} | 多头平均持仓天数: {round(self.obj["long_avg_holding"], 2)} | 空头平均持仓天数: {round(self.obj["short_avg_holding"], 2)} ')
            file.write('</div>\n')
    ...
    def cal_daily_fee(self):
        ...
        for i, doc in daily_list.iterrows():
            ...
            equity_trade = self.order_df[
                (self.order_df['model_flag'] == self.model_flag) & (self.order_df['trading_date'] == doc['date'])]
    ...
    def calc_winning(self):
        ...
        self.obj['long_loss_count'] = len(
            long_df[(long_df['price'] - long_df['openPrice']) * long_df['buyOrSell'] <= 0])
        if (self.obj['long_win_count'] + self.obj['long_loss_count']) == 0:
            self.obj['long_win_rate'] = 0
        else:
            self.obj['long_win_rate'] = self.obj['long_win_count'] / (
                        self.obj['long_win_count'] + self.obj['long_loss_count'])

        # 空头
        short_df = df[df['buyOrSell'] == SELL]
        self.obj['short_win_count'] = len(
            short_df[(short_df['price'] - short_df['openPrice']) * short_df['buyOrSell'] > 0])
        self.obj['short_loss_count'] = len(
            short_df[(short_df['price'] - short_df['openPrice']) * short_df['buyOrSell'] <= 0])
        if (self.obj['short_win_count'] + self.obj['short_loss_count']) == 0:
            self.obj['short_win_rate'] = 0
        else:
            self.obj['short_win_rate'] = self.obj['short_win_count'] / (
                        self.obj['short_win_count'] + self.obj['short_loss_count'])
        pass
    ...
    def calc_tp_lp(self):
        ...
        self.obj['long_win_profit'] = (
                    (long_win_df['price'] - long_win_df['openPrice']) * long_win_df['buyOrSell'] * long_win_df[
                'openOrClose'] * long_win_df['count_back'] * long_win_df['mul'] - long_win_df['fee']).sum()
        # 多头损失
        long_loss_df = long_df[((long_df['price'] - long_df['openPrice']) * long_df['buyOrSell']) <= 0]
        self.obj['long_loss_profit'] = (
                    (long_loss_df['price'] - long_loss_df['openPrice']) * long_loss_df['buyOrSell'] * long_loss_df[
                'openOrClose'] * long_loss_df['count_back'] * long_loss_df['mul'] - long_loss_df['fee']).sum()

        # 空头
        short_df = df[df['buyOrSell'] == SELL]
        # 空头盈利
        short_win_df = short_df[((short_df['price'] - short_df['openPrice']) * short_df['buyOrSell']) > 0]
        self.obj['short_win_profit'] = (
                    (short_win_df['price'] - short_win_df['openPrice']) * short_win_df['buyOrSell'] * short_win_df[
                'openOrClose'] * short_win_df['count_back'] * short_win_df['mul'] - short_win_df['fee']).sum()
        # 空头损失
        short_loss_df = short_df[((short_df['price'] - short_df['openPrice']) * short_df['buyOrSell']) <= 0]
        self.obj['short_loss_profit'] = (
                    (short_loss_df['price'] - short_loss_df['openPrice']) * short_loss_df['buyOrSell'] * short_loss_df[
                'openOrClose'] * short_loss_df['count_back'] * short_loss_df['mul'] - short_loss_df['fee']).sum()

    def add_holding_days(self):
        """
        添加持仓天数
        """
        close_orders = self.order_df[
            (self.order_df["model_flag"] == self.model_flag) & (self.order_df["openOrClose"] == CLOSE)]
        for idx, doc in close_orders.iterrows():
            open_order = \
            self.order_df[(self.order_df['tradeId'] == doc['ID']) & (self.order_df['openOrClose'] == OPEN)].iloc[0]
    ...
    def calc_holding(self):
        ...
        self.obj['long_holding'] = self.order_df[
            (self.order_df['buyOrSell'] == BUY) & (self.order_df['openOrClose'] == CLOSE) & (
                        self.order_df['model_flag'] == self.model_flag)]["holding_days"].sum()

        self.obj['short_holding'] = self.order_df[
            (self.order_df['buyOrSell'] == SELL) & (self.order_df['openOrClose'] == CLOSE) & (
                        self.order_df['model_flag'] == self.model_flag)]["holding_days"].sum()
