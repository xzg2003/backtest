# 回测框架文档
## 一、工作流
- 1. 在 `main.py` 中调用 `./backtesting/prepare_data.py`，获取合并后的数据
- 2. 使用模型训练和预测，生成预测值，保存在 `./data/pre.pkl` 中（可选）
- 3. 通过 `strategy.py` 产生仓位信息，保存在 `./data/position.pkl` 中（可以在 `pre.pkl` 的基础上生成，也可以直接根据市场行情生成）
- 4. 输入 `position.pkl`，通过 `./backtesting/gen_order.py` 产生的交易记录，保存在 `./data/order.pkl` 中，包含每天的交易方向，开仓、平仓动作，手续费
- 5. 通过 `./backtesting/perfomance4.py` 计算交易的指标，并生成回测结果网页

## 二、文件管理
### 目录结构
- 1. `./data` 存放运算过程中产生的预测数据、交易记录
- 2. `./factor_cal` 存放因子计算代码
- 3. `./backtesting` 存放回测代码
- 4. `./model` 存放模型文件
- 5. `./strategy` 存放策略文件
