...
class factor_calculator:
    ...
    def factors_cal(self):
        ...
        for instrument in self.instruments:
            # 构建数据路径data_path
            data_path = f'{DATA_DIR}/{self.k_line_type}/{instrument}/{instrument}.csv'
            ...
            for factor_name, calculator in self.factors_dict.items():
                try:
                    if factor_name=='Tr':
                        # Tr 因子不需要滑动长度，因此单独处理
                        save_path = f"{DATA_DIR}/{self.k_line_type}/{instrument}/{factor_name}.csv"
                        ...
                        if os.path.exists(f'{DATA_DIR}/{self.k_line_type}/{instrument}/Tr.csv'):
                            tr = pd.read_csv(f'{DATA_DIR}/{self.k_line_type}/{instrument}/Tr.csv')
                            tr = tr['Tr']
                        else:
                            tr = None
                        ...
                        for length in self.lengths:
                            ...
                            save_path = f'{DATA_DIR}/{self.k_line_type}/{instrument}/{factor_name}@{length}.csv'
...
if __name__ == "__main__":
    ...
    #steps = [1,2,4,8,12,15,20,24,30,40,60,120]
    # 创建因子计算器实例，以调用内部的函数
